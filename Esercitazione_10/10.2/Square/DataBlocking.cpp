...
//Devizione standard per il problema del traveling salesman
double ErrorVector(vector<double> vector) {
	double sum = 0;
	for(unsigned int i=0; i<vector.size(); i++) sum += pow(Mean(vector)-vector.at(i), 2.);
	return sum/(vector.size()-1);
}
...
void AnalysisMatrix(int NumberOfBlocks, vector<vector <double>> MatrixOfData, double ScaleFactor, double IndexTraslation, const char* OutputFile){
	...
	int NumberOfLines = MatrixOfData.size(); 	//Numero di vettori su cui si fa il data blocking
	int NumberOfData = MatrixOfData[0].size(); 
	int NumberOfDataPerBlocks = int(NumberOfData/NumberOfBlocks); 

	//Data blocking su ogni riga della matriche
	for(int j=0; j<NumberOfLines; j++){ 

		vector<double> ai(NumberOfBlocks);
		vector<double> ai_2(NumberOfBlocks);
	 	double temp; 							// variabile di appoggio

	  	for(int i=0; i<NumberOfBlocks;i++){
			temp = Sum(MatrixOfData[j],i*NumberOfDataPerBlocks,(i+1)*NumberOfDataPerBlocks)/(NumberOfDataPerBlocks);
			ai.at(i) = temp;
			ai_2.at(i) = pow(temp,2);
		}

	//Calcolo la media progressiva del vettore mean e le deviazione standard della media progressiva
	 	vector<double> mean_prog(NumberOfBlocks);
		vector<double> mean_2_prog(NumberOfBlocks);
		vector<double> mean_std(NumberOfBlocks);
		for(int i=0;i<NumberOfBlocks;i++){
	...
	//Stampo su file i risultati nel seguente formato: 
	//indice, media progressiva, deviazione standard della media progressiva con il massimo numero di blocchi
		if (results.is_open()){
	   		results << j*ScaleFactor - IndexTraslation << " "<< mean_prog.at(NumberOfBlocks-1) << " " <<mean_std.at(NumberOfBlocks-1)<< endl;
	  	 }
