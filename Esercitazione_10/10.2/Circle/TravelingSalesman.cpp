...
//Costruttore
TravelingSalesman::TravelingSalesman(int NumberOfCities, int NumberOfParents, int NumberOfGenerations, int Seed): _FitnessDistances(NumberOfParents){
...
//Metodo che stampa le posizioni
void TravelingSalesman::PrintPositions(void){
  cout << "x  y" << endl;
  for(int j=0; j<_NumberOfCities;j++){
    for (int i=1; i<3 ; i++) {
      cout<<_Positions[j][i]<<"  ";
...
void TravelingSalesman::RunMetropolis(double Beta, int NumberOfMutations) {
    ...
    _BestCities.push_back(_StringsOfCities[_BestIndex]);
	}
}
...
//Metodo per il RandomSearch
void TravelingSalesman::RandomSearch(void){
	for(int i=0; i<_NumberOfParents; i++){
...
//Metodo che resituisce la distanza migliore
double TravelingSalesman::MinimumDistance(void) {
	int index_min = min_element(_RememberBestDistance.begin(),_RememberBestDistance.end()) - _RememberBestDistance.begin();
	double minimum = _RememberBestDistance[index_min];
	return minimum;
}
...
vector<vector<double>> TravelingSalesman::GetPositions(void){
  return _Positions;
}