#ifndef _TravelingSalesman_h
#define _TravelingSalesman_h
...
private:
  ...
  vector<vector<int>> _BestCities;            //Matrice delle migliori città per ogni passo metropolis

public:

  TravelingSalesman(int NumberOfCity, int NumberOfParents, int NumberOfGenerations, int Seed);
  ~TravelingSalesman();
  ...
  void PrintPositions(void);                   //Stampa a video le posizioni
  ...
  void RandomSearch(void);                      //Permutazione delle stringhe
  double MinimumDistance(void);                 //Restituisce la distanza minima registrata in una run
  vector<vector<double>> Get_Positions(void);   //Restituisce la matrice delle posizioni delle città
  int* BestWalk(void);                          //Restituisce il genitore con il percorso migliore di una run
