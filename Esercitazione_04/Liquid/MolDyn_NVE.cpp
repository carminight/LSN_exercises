...
int main(){
  ...
  int thermalizationSteps = 5000;
  ofstream Term;
  Term.open("../Results/Liquid/output_restart.out",ios::app);
  ...
  for(int iblk=1; iblk <= nblk; ++iblk){ 
      ...
      if(istep%10 == 0){
        ...
        //Valori istanntanei delle grandezze
        //Print_Istant(); 
...
void Input(){
  ...
  ReadInput >> iprint;
  ReadInput >> nblk;
  ReadInput >> mode; //Per decidere quale configurazione leggere all'inizio

  cout << "Number of blocks for data blocking = " << nblk << endl;

  //Nel caso si volessero aggiungere le tail corrections si scommentino le seguenti righe
  vtail = 0.; //(8.0*M_PI*rho)/(9.0*pow(rcut,9)) - (8.0*M_PI*rho)/(3.0*pow(rcut,3));
  ptail = 0.; //(32.0*M_PI*rho)/(9.0*pow(rcut,9)) - (16.0*M_PI*rho)/(3.0*pow(rcut,3));
  ...
  if(mode == 0){ //Read initial configuration fcc
  ...
  if(mode == 2){
    cout << "Read oldold configuration from file old.final " << endl << endl;
...
void OldConfig1(void){ //Write oldold configuration
  ofstream OldConfig1;

  cout << "Print old configuration to file old.0 " << endl << endl;
  OldConfig1.open("old.0");

  for (int i=0; i<npart; ++i){
    OldConfig1 << xold[i]/box << "   " <<  yold[i]/box << "   " << zold[i]/box << endl;
  }
  OldConfig1.close();
...
void OldOldConfig(void){ //Write oldold configuration
  ofstream OldOldConfig;

  cout << "Print old old configuration to file old.final " << endl << endl;
  OldOldConfig.open("old.final");

  for (int i=0; i<npart; ++i){
    OldOldConfig << xold2[i]/box << "   " <<  yold2[i]/box << "   " << zold2[i]/box << endl;
  }
  OldOldConfig.close();
...
void Print_Istant(){  //Stampa i valori istantanei di pressione, energia potenziale, temperature, energia totale, energia cinetica
  ...
  Epot.open("../Results/Liquid/output_epot.dat",ios::app);
  Pres.open("../Results/Liquid/output_pres.dat",ios::app);
  Temp.open("../Results/Liquid/output_temp.dat",ios::app);
  Etot.open("../Results/Liquid/output_etot.dat",ios::app);
  Ekin.open("../Results/Liquid/output_ekin.dat",ios::app);
...
void Averages(int iblk){ //Print results for current block
  ...
  ofstream Epot, Pres,Temp,Ekin,Etot;
  Epot.open("../Results/Liquid/ave_epot.out",ios::app);
  Pres.open("../Results/Liquid/ave_pres.out",ios::app);
  Temp.open("../Results/Liquid/ave_temp.out",ios::app);
  Ekin.open("../Results/Liquid/ave_ekin.out",ios::app);
  Etot.open("../Results/Liquid/ave_etot.out",ios::app);
